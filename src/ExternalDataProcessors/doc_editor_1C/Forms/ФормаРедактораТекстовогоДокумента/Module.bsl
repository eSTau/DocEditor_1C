...
Процедура ДобавитьОписаниеПрограммныхМодулей(ОбъектМетаданныхСтруктуры, ТолькоПрограммныйИнтерфейс = Истина)
	...
	Если ОбъектМетаданныхСтруктуры.Свойства.Свойство("Модули") Тогда
		Для Каждого МодуляОбъекта Из ОбъектМетаданныхСтруктуры.Свойства.Модули Цикл
			КонтекстМодуля = СтруктураКонтекстМодуляОбъекта(МодуляОбъекта.Значение);
			Если Не КонтекстМодуля = Неопределено Тогда
				ДобавитьОписаниеКонтекстаМодуля(КонтекстМодуля, МодуляОбъекта.Представление, ТолькоПрограммныйИнтерфейс);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ДобавитьОписаниеВозвращаемогоЗначения(ОписаниеМетода)
	...
	Если ОписаниеМетода.ВозвращаемыеЗначения = Неопределено 
		И ПустаяСтрока(ОписаниеМетода.ПроизвольноеОписаниеВозвращаемыеЗначения) Тогда
...
Процедура ДобавитьОписаниеПримеров(ОписаниеМетода)
	...
	БлокОписанияПримера = БлокОписанияПримера 
		+ ДокументБлокПереносСтроки() + ОписаниеМетода.Примеры;
...
Процедура ДобавитьОписаниеМетода(ОписаниеМетода)
	...
	БлокОписанияМетода = БлокОписанияМетода 
		+ ДокументБлокПереносСтроки() + ОписаниеМетода.Назначение;
...
Функция СвойстваПланаСчетов(ИмяПланаСчетов)
	...
	СвойстваПланаСчетов.Модули.Добавить("ПланСчетов." + ИмяПланаСчетов 
		+ ".МодульОбъекта.txt", "Модуль объекта: " + ИмяПланаСчетов);
	СвойстваПланаСчетов.Модули.Добавить("ПланСчетов." + ИмяПланаСчетов 
		+ ".МодульМенеджера.txt", "Модуль менеджера: " + ИмяПланаСчетов);
...
Функция СвойстваРегистраБухгалтерии(ИмяРегистраБухгалтерии)
	...
	СвойстваРегистраБухгалтерии.Модули.Добавить("РегистрБухгалтерии." + ИмяРегистраБухгалтерии + ".МодульНабораЗаписей.txt", 
		"Модуль набора записей: " + ИмяРегистраБухгалтерии);
...
Функция СвойстваБизнесПроцесса(ИмяБизнесПроцесса)
	...
	СвойстваБизнесПроцесса.Модули.Добавить("БизнесПроцесс." + ИмяБизнесПроцесса 
		+ ".МодульОбъекта.txt", "Модуль объекта: " + ИмяБизнесПроцесса);
	СвойстваБизнесПроцесса.Модули.Добавить("БизнесПроцессы." + ИмяБизнесПроцесса 
		+ ".МодульМенеджера.txt", "Модуль менеджера: " + ИмяБизнесПроцесса);
