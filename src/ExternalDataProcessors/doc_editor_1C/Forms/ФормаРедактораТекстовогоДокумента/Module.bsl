...
Процедура ДобавитьКлассДокументы()
	...
	ДанныеОбъекта = ДанныеОбъектаМетаданных("Документы");
...
Процедура ДобавитьНовыйКлассОбъектов(ЭлементРодитель, ИмяМенеджераКласса, ИмяКласса, СинонимКласса)	
	...
	ДанныеОбъекта = ДанныеОбъектаМетаданных(ИмяМенеджераКласса);
...
&НаСервере
Функция ДанныеОбъектаМетаданных(ИмяМенеджераКласса)
...
//	КонецЕсли;
	...
	МаркерФорматирования = Новый Структура("Маркер, Представление, Объект", 
		ВидРазработки + ПредставлениеОбъекта.Текст, ПредставлениеОбъекта.ВыделяемыйФрагмент, ОбъектМетаданных);
		
	ДобавитьМаркерВТекст(МаркерФорматирования, Истина);
	
	ДобавитьСвойстваОбъектаВОписание(ОбъектМетаданных);
	
	ДобавитьОписаниеПодчиненныхОбъектов(ОбъектМетаданных);
...
&НаКлиенте
Процедура ДобавитьСвойстваОбъектаВОписание(ОбъектМетаданных)
	
	Если Не ЗначениеЗаполнено(ОбъектМетаданных.Свойства) Тогда
		ЗаполнитьСвойстваОбъектаМетаданных(ОбъектМетаданных);	
	КонецЕсли;
	
	СвойстваОбъекта = ОбъектМетаданных.Свойства;
	...
	ДобавитьЗаголовокТаблицыСвойств();
	Для Каждого СвойствоОбъекта Из СвойстваОбъекта Цикл
		Если СвойствоОбъекта.Ключ = "Описание" Тогда
			Продолжить;
		КонецЕсли;
		
		ДобавитьНовуюСтрокуТаблицы();
		ДобавитьВТаблицуСтрокуОписанияСвойства(СвойствоОбъекта.Ключ, СвойствоОбъекта.Значение);
...
&НаКлиенте
Процедура ЗаполнитьСвойстваОбъектаМетаданных(ОбъектМетаданных)
	ОбъектМетаданных.Свойства = СвойстаОбъектаМетаданных(ОбъектМетаданных.Имя, ОбъектМетаданных.Класс);	
КонецПроцедуры
...
&НаКлиенте
Процедура ДобавитьОписаниеПодчиненныхОбъектов(ОбъектМетаданных)
	...
	Для Каждого ПодчиненныйЭлемент Из ОбъектМетаданных.ПолучитьЭлементы() Цикл
		...
		Если ЭтоПодчиненныеЭлементыОбъекта(ПодчиненныйЭлемент) Тогда
			...
			ДобавитьЗаголовокТаблицы();
			ДобавитьОписаниеПолейОбъекта(ОбъектМетаданных, ПодчиненныйЭлемент);
		...
		ИначеЕсли (ПодчиненныйЭлемент.Класс = "ТабличнаяЧасть" 
			...
			Для Каждого ТабличнаяЧастьОбъекта Из ПодчиненныйЭлемент.ПолучитьЭлементы() Цикл
				...
				ДобавитьЗаголовокТаблицы();
				ДобавитьОписаниеПолейОбъекта(ОбъектМетаданных, ТабличнаяЧастьОбъекта);
...
&НаКлиенте
Процедура ДобавитьЗаголовокТаблицыСвойств()
...
&НаКлиенте
Процедура ДобавитьВТаблицуСтрокуОписанияСвойства(ИмяСвойства, ЗначениеСвойства)
...
&НаКлиенте
Процедура ДобавитьЗаголовокТаблицы()
...
// TODO: Заменить наименование процедуры она выводит не только поля но и формы, команды и макеты
&НаКлиенте
Процедура ДобавитьОписаниеПолейОбъекта(ОбъектМетаданных, МенеджерПолей)
	
	Для Каждого Поле Из МенеджерПолей.ПолучитьЭлементы() Цикл
		
		Если Не ЗначениеЗаполнено(Поле.Свойства) Тогда
			ЗаполнитьСвойстваПодчиненногоЭлемента(ОбъектМетаданных, Поле);
		КонецЕсли;
		...
		ДобавитьНовуюСтрокуТаблицы();
		ДобавитьВТаблицуСтрокуОписанияДанных(Поле.Имя, ПредставлениеТипа, Поле.Описание, Поле);
...
&НаКлиенте
Процедура ЗаполнитьСвойстваПодчиненногоЭлемента(ОбъектМетаданных, Поле)
	
	Если Поле.Класс = "Реквизит" Тогда
		Поле.Свойства = СвойстваРеквизита("Справочники", "ИмяРеквизита");
	КонецЕсли;
		
КонецПроцедуры

&НаСервереБезКонтекста
Функция СвойстваРеквизита(КлассОбъекта, ИмяРеквизита)
	
	ОбъектМетаданных = Метаданные["Справочники"]["тест_Справочник"];
	РеквизитОбъекта = ОбъектМетаданных.Реквизиты["РеквизитСтрока"];
...
&НаКлиенте
Процедура ДобавитьНовуюСтрокуТаблицы()
