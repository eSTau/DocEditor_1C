...
&НаКлиенте
Процедура КомандаДобавитьДетальноеОписаниеОбъекта(Команда)
	
	ДобавитьОписаниеОбъектаВДокумент();
	
	Элементы.СтруктураМетаданных.Развернуть(ТекущийОбъектМетаданных().ПолучитьИдентификатор()); 
...
Процедура НастроитьКомандыКонтекстногоМеню(ОбъектМетаданных)
	...
	НастроитьКнопкуДетальноеОписаниеОбъекта(ОбъектМетаданных);
...
&НаКлиенте
Процедура НастроитьКнопкуДетальноеОписаниеОбъекта(ОбъектМетаданныхСтруктуры)
	
	Возврат;
	
	Если (ЭтоСлужебныйЭлемент(ОбъектМетаданныхСтруктуры) 
		Или ОбъектМетаданныхСтруктуры.МенеджерОбъектов
		Или ОбъектМетаданныхСтруктуры.Класс = "Подсистема") Тогда

		Возврат;		

	КонецЕсли;
	
	Если ОбъектМетаданныхСтруктуры.ПолучитьЭлементы().Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Элементы.ДобавитьДетальноеОписаниеОбъекта.Видимость = Истина;
...
Функция ОбъектИмеетПрограммныйМодульОбъекта(ОбъектМетаданных)
	...
	Возврат ОбъектМетаданных.Класс = "МодульОбычногоПриложения" 
		...
		Или ОбъектМетаданных.Класс = "ОбщийМодуль" 
		Или ОбъектМетаданных.Класс = "ПланОбмена"
		Или ОбъектМетаданных.Класс = "ОбщаяФорма" 
		Или ОбъектМетаданных.Класс = "ОбщаяКоманда"
		...
		Или ОбъектМетаданных.Класс = "Форма" 
		Или ОбъектМетаданных.Класс = "Команда";
...
Процедура ДобавитьФормуОбъектаМетаданных(ИмяМенеджераКласса, ЭлементРодитель, ОбъектМетаданных)
	...
	ОбъектыСФормами.Добавить("ХранилищаНастроек");
...
Функция ЭтоПрикладнойОбъект(ОбъектМетаданных)
	...
	Возврат ОбъектМетаданных.Класс = "ПланОбмена" Или ОбъектМетаданных.Класс = "КритерийОтбора"
		...
		Или ОбъектМетаданных.Класс = "ВнешнийИсточникДанных" Или ОбъектМетаданных.Класс = "ХранилищеНастроек";
...
Функция СвойстваОбъектаМетаданных(ИмяОбъекта, КлассОбъекта)
	...
	ИначеЕсли КлассОбъекта = "ФункциональнаяОпция" Тогда
		Возврат СвойстваФункциональнойОпции(ИмяОбъекта);
	ИначеЕсли КлассОбъекта = "ПараметрФункциональнойОпции" Тогда
		Возврат СвойстваПараметраФункциональнойОпции(ИмяОбъекта);
	ИначеЕсли КлассОбъекта = "ОпределяемыйТип" Тогда
		Возврат СвойстваОпределяемогоТипа(ИмяОбъекта);
	ИначеЕсли КлассОбъекта = "ХранилищеНастроек" Тогда
		Возврат СвойстваХранилищаНастроек(ИмяОбъекта);
	ИначеЕсли КлассОбъекта = "ОбщаяФорма" Тогда
		Возврат СвойстваОбщейФормы(ИмяОбъекта);
	ИначеЕсли КлассОбъекта = "ОбщаяКоманда" Тогда
		Возврат СвойстваОбщейКоманды(ИмяОбъекта);
	ИначеЕсли КлассОбъекта = "ГруппаКоманд" Тогда
		Возврат СвойстваГруппыКомпнд(ИмяОбъекта);
	ИначеЕсли КлассОбъекта = "ОбщийМакет" Тогда
		Возврат СвойстваОбщегоМакета(ИмяОбъекта);
	ИначеЕсли КлассОбъекта = "ПакетXDTO" Тогда
		Возврат СвойстваПакетаXDTO(ИмяОбъекта);
	ИначеЕсли КлассОбъекта = "WebСервис" Тогда
		Возврат СвойстваWebСервиса(ИмяОбъекта);
	ИначеЕсли КлассОбъекта = "HTTPСервис" Тогда
		Возврат СвойстваHttpСервиса(ИмяОбъекта);
	ИначеЕсли КлассОбъекта = "WSСсылка" Тогда
		Возврат СвойстваWSСсылка(ИмяОбъекта);
	...
	Иначе
		...
		СвойстваОбъекта.Вставить("Описание", Новый Структура);
		СвойстваОбъекта.Описание.Вставить("Комментарий", "Описание не реализовано");
		СвойстваОбъекта.Описание.Вставить("Пояснение", "");
...
&НаСервереБезКонтекста
Функция СвойстваWSСсылка(ИмяWSСсылки)
	
	СвойстваWSСсылки = Новый Структура;

	ОбъектМетаданных = Метаданные.WSСсылки[ИмяWSСсылки];

	СвойстваWSСсылки.Вставить("Описание", Новый Структура);
	СвойстваWSСсылки.Описание.Вставить("Комментарий", ОбъектМетаданных.Комментарий);
	СвойстваWSСсылки.Описание.Вставить("Пояснение", "");

	СвойстваWSСсылки.Вставить("Синоним", ОбъектМетаданных.Синоним);
	СвойстваWSСсылки.Вставить("URLИсточника", ОбъектМетаданных.URLИсточника);
	СвойстваWSСсылки.Вставить("WSОпределение", ОбъектМетаданных.WSОпределение);

	ПреобразоватьЗначенияСвойствВСтроковоеПредставление(СвойстваWSСсылки);
	
	Возврат СвойстваWSСсылки;
	
КонецФункции

&НаСервереБезКонтекста
Функция СвойстваHttpСервиса(ИмяHttpСервиса)
	
	СвойстваHttpСервиса = Новый Структура;

	ОбъектМетаданных = Метаданные.HTTPСервисы[ИмяHttpСервиса];

	СвойстваHttpСервиса.Вставить("Описание", Новый Структура);
	СвойстваHttpСервиса.Описание.Вставить("Комментарий", ОбъектМетаданных.Комментарий);
	СвойстваHttpСервиса.Описание.Вставить("Пояснение", "");

	СвойстваHttpСервиса.Вставить("Синоним", ОбъектМетаданных.Синоним);
	СвойстваHttpСервиса.Вставить("КорневойURL", ОбъектМетаданных.КорневойURL);
	СвойстваHttpСервиса.Вставить("ПовторноеИспользованиеСеансов", ОбъектМетаданных.ПовторноеИспользованиеСеансов);
	СвойстваHttpСервиса.Вставить("ВремяЖизниСеанса", ОбъектМетаданных.ВремяЖизниСеанса);

	ПреобразоватьЗначенияСвойствВСтроковоеПредставление(СвойстваHttpСервиса);
	
	СвойстваHttpСервиса.Вставить("Модули", Новый СписокЗначений);
	СвойстваHttpСервиса.Модули.Добавить(
		"HTTPСервис." + ИмяHttpСервиса + ".Модуль.txt", 
		"Модуль http сервис: " + ИмяHttpСервиса);

	Возврат СвойстваHttpСервиса;
	
КонецФункции

&НаСервереБезКонтекста
Функция СвойстваWebСервиса(ИмяWebСервиса)
	
	СвойстваWebСервиса = Новый Структура;

	ОбъектМетаданных = Метаданные.WebСервисы[ИмяWebСервиса];

	СвойстваWebСервиса.Вставить("Описание", Новый Структура);
	СвойстваWebСервиса.Описание.Вставить("Комментарий", ОбъектМетаданных.Комментарий);
	СвойстваWebСервиса.Описание.Вставить("Пояснение", "");

	СвойстваWebСервиса.Вставить("Синоним", ОбъектМетаданных.Синоним);
	СвойстваWebСервиса.Вставить("ПакетыXDTO", ОбъектМетаданных.ПакетыXDTO);
	СвойстваWebСервиса.Вставить("ПространствоИмен", ОбъектМетаданных.ПространствоИмен);
	СвойстваWebСервиса.Вставить("ИмяФайлаПубликации", ОбъектМетаданных.ИмяФайлаПубликации);
	СвойстваWebСервиса.Вставить("ПовторноеИспользованиеСеансов", ОбъектМетаданных.ПовторноеИспользованиеСеансов);
	СвойстваWebСервиса.Вставить("ВремяЖизниСеанса", ОбъектМетаданных.ВремяЖизниСеанса);

	ПреобразоватьЗначенияСвойствВСтроковоеПредставление(СвойстваWebСервиса);
	
	СвойстваWebСервиса.Вставить("Модули", Новый СписокЗначений);
	СвойстваWebСервиса.Модули.Добавить(
		"WebСервис." + ИмяWebСервиса + ".Модуль.txt", 
		"Модуль web сервис: " + ИмяWebСервиса);

	Возврат СвойстваWebСервиса;
	
КонецФункции

&НаСервереБезКонтекста
Функция СвойстваПакетаXDTO(ИмяПакетаXDTO)
	
	СвойстваПакетаXDTO = Новый Структура;

	ОбъектМетаданных = Метаданные.ПакетыXDTO[ИмяПакетаXDTO];

	СвойстваПакетаXDTO.Вставить("Описание", Новый Структура);
	СвойстваПакетаXDTO.Описание.Вставить("Комментарий", ОбъектМетаданных.Комментарий);
	СвойстваПакетаXDTO.Описание.Вставить("Пояснение", "");

	СвойстваПакетаXDTO.Вставить("Синоним", ОбъектМетаданных.Синоним);
	СвойстваПакетаXDTO.Вставить("ПространствоИмен", ОбъектМетаданных.ПространствоИмен);
	СвойстваПакетаXDTO.Вставить("Пакет", ОбъектМетаданных.Пакет);

	ПреобразоватьЗначенияСвойствВСтроковоеПредставление(СвойстваПакетаXDTO);

	Возврат СвойстваПакетаXDTO;
	
КонецФункции

&НаСервереБезКонтекста
Функция СвойстваОбщегоМакета(ИмяОбщегоМакета)
	
	СвойстваОбщегоМакета = Новый Структура;

	ОбъектМетаданных = Метаданные.ОбщиеМакеты[ИмяОбщегоМакета];

	СвойстваОбщегоМакета.Вставить("Описание", Новый Структура);
	СвойстваОбщегоМакета.Описание.Вставить("Комментарий", ОбъектМетаданных.Комментарий);
	СвойстваОбщегоМакета.Описание.Вставить("Пояснение", "");

	СвойстваОбщегоМакета.Вставить("Синоним", ОбъектМетаданных.Синоним);
	СвойстваОбщегоМакета.Вставить("ТипМакета", ОбъектМетаданных.ТипМакета);

	ПреобразоватьЗначенияСвойствВСтроковоеПредставление(СвойстваОбщегоМакета);

	Возврат СвойстваОбщегоМакета;
	
КонецФункции

&НаСервереБезКонтекста
Функция СвойстваГруппыКомпнд(ИмяГруппыКоманд)
	
	СвойстваГруппыКоманд = Новый Структура;

	ОбъектМетаданных = Метаданные.ГруппыКоманд[ИмяГруппыКоманд];

	СвойстваГруппыКоманд.Вставить("Описание", Новый Структура);
	СвойстваГруппыКоманд.Описание.Вставить("Комментарий", ОбъектМетаданных.Комментарий);
	СвойстваГруппыКоманд.Описание.Вставить("Пояснение", ОбъектМетаданных.Подсказка);

	СвойстваГруппыКоманд.Вставить("Синоним", ОбъектМетаданных.Синоним);
	СвойстваГруппыКоманд.Вставить("Категория", ОбъектМетаданных.Категория);
	СвойстваГруппыКоманд.Вставить("Отображение", ОбъектМетаданных.Отображение);

	ПреобразоватьЗначенияСвойствВСтроковоеПредставление(СвойстваГруппыКоманд);

	Возврат СвойстваГруппыКоманд;
	
КонецФункции

&НаСервереБезКонтекста
Функция СвойстваОбщейКоманды(ИмяОбщейКоманды)
	
	СвойстваОбщейКоманды = Новый Структура;

	ОбъектМетаданных = Метаданные.ОбщиеКоманды[ИмяОбщейКоманды];

	СвойстваОбщейКоманды.Вставить("Описание", Новый Структура);
	СвойстваОбщейКоманды.Описание.Вставить("Комментарий", ОбъектМетаданных.Комментарий);
	СвойстваОбщейКоманды.Описание.Вставить("Пояснение", ОбъектМетаданных.Подсказка);

	СвойстваОбщейКоманды.Вставить("Синоним", ОбъектМетаданных.Синоним);
	СвойстваОбщейКоманды.Вставить("ГруппыКоманды", ОбъектМетаданных.Группа);
	СвойстваОбщейКоманды.Вставить("ТипПараметраКоманды", ОбъектМетаданных.ТипПараметраКоманды);
	СвойстваОбщейКоманды.Вставить("РежимИспользованияПараметра", ОбъектМетаданных.РежимИспользованияПараметра);
	СвойстваОбщейКоманды.Вставить("ИзменяетДанные", ОбъектМетаданных.ИзменяетДанные);
	СвойстваОбщейКоманды.Вставить("Отображение", ОбъектМетаданных.Отображение);
	СвойстваОбщейКоманды.Вставить("СочетаниеКлавиш", ОбъектМетаданных.СочетаниеКлавиш);

	ПреобразоватьЗначенияСвойствВСтроковоеПредставление(СвойстваОбщейКоманды);

	СвойстваОбщейКоманды.Вставить("Модули", Новый СписокЗначений);
	СвойстваОбщейКоманды.Модули.Добавить(
		"ОбщаяКоманда." + ИмяОбщейКоманды + ".МодульКоманды.txt", 
		"Модуль команды: " + ИмяОбщейКоманды);

	Возврат СвойстваОбщейКоманды;

КонецФункции

&НаСервереБезКонтекста
Функция СвойстваХранилищаНастроек(ИмяХранилищаНастроека)

	СвойстваХранилищаНастроек = Новый Структура;

	ОбъектМетаданных = Метаданные.ХранилищаНастроек[ИмяХранилищаНастроека];

	СвойстваХранилищаНастроек.Вставить("Описание", Новый Структура);
	СвойстваХранилищаНастроек.Описание.Вставить("Комментарий", ОбъектМетаданных.Комментарий);
	СвойстваХранилищаНастроек.Описание.Вставить("Пояснение", "");

	СвойстваХранилищаНастроек.Вставить("Синоним", ОбъектМетаданных.Синоним);

	ПреобразоватьЗначенияСвойствВСтроковоеПредставление(СвойстваХранилищаНастроек);

	СвойстваХранилищаНастроек.Вставить("Модули", Новый СписокЗначений);
	СвойстваХранилищаНастроек.Модули.Добавить(
		"ХранилищеНастроек." + ИмяХранилищаНастроека + ".МодульМенеджера.txt", 
		"Модуль менеджера: " + ИмяХранилищаНастроека);

	Возврат СвойстваХранилищаНастроек;
	
КонецФункции

&НаСервереБезКонтекста
Функция СвойстваОпределяемогоТипа(ИмяОпределяемогоТипа)
	
	СвойстваОпределяемогоТипа = Новый Структура;

	ОбъектМетаданных = Метаданные.ОпределяемыеТипы[ИмяОпределяемогоТипа];

	СвойстваОпределяемогоТипа.Вставить("Описание", Новый Структура);
	СвойстваОпределяемогоТипа.Описание.Вставить("Комментарий", ОбъектМетаданных.Комментарий);
	СвойстваОпределяемогоТипа.Описание.Вставить("Пояснение", "");

	СвойстваОпределяемогоТипа.Вставить("Синоним", ОбъектМетаданных.Синоним);
	СвойстваОпределяемогоТипа.Вставить("Тип", ОбъектМетаданных.Тип);

	ПреобразоватьЗначенияСвойствВСтроковоеПредставление(СвойстваОпределяемогоТипа);

	Возврат СвойстваОпределяемогоТипа;
...
Функция СвойстваПодсистемы(ИмяПодсистемы)
	...
	СвойстваПодсистемы.Вставить("Синоним", ОбъектМетаданных.Синоним);
...
Функция СвойстваОбщегоМодуля(ИмяОбщегоМодуля)
	...
	СвойстваОбщегоМодуля.Вставить("Синоним", ОбъектМетаданных.Синоним);
...
Функция СвойстваПараметраСеанса(ИмяПараметраСеанса)
	...
	СвойстваПараметраСеанса.Вставить("Синоним", ОбъектМетаданных.Синоним);
...
Функция СвойстваРоли(ИмяРоли)
	...
	СвойстваРоли.Вставить("Синоним", ОбъектМетаданных.Синоним);
...
Функция СвойстваОбщегоРеквизита(ИмяОбщегоРеквизита)
	...
	СвойстваОбщегоРеквизита.Вставить("Синоним", ОбъектМетаданных.Синоним);
...
Функция СвойстваПланаОбмена(ИмяПланаОбмена)
	...
	СвойстваПланаОбмена.Вставить("Синоним", ОбъектМетаданных.Синоним);
...
Функция СвойстваКритерияОтбора(ИмяКритерияОтбора)
	...
	СвойстваКритерияОтбора.Вставить("Синоним", ОбъектМетаданных.Синоним);
...
Функция СвойстваПодпискиНаСобытие(ИмяПодпискиНаСобытие)
	...
	СвойстваПодпискиНаСобытие.Вставить("Синоним", ОбъектМетаданных.Синоним);
...
Функция СвойстваРегламентногоЗадания(ИмяРегламентногоЗадания)
	...
	СвойстваРегламентногоЗадания.Вставить("Синоним", ОбъектМетаданных.Синоним);
	СвойстваРегламентногоЗадания.Вставить("ГруппаОсновные", "Основные");
	СвойстваРегламентногоЗадания.Вставить("Имя", ОбъектМетаданных.Имя);
...
&НаСервереБезКонтекста
Функция СвойстваФункциональнойОпции(ИмяФункциональнойОпции)
	
	СвойстваФункциональнойОпции = Новый Структура;

	ОбъектМетаданных = Метаданные.ФункциональныеОпции[ИмяФункциональнойОпции];

	СвойстваФункциональнойОпции.Вставить("Описание", Новый Структура);
	СвойстваФункциональнойОпции.Описание.Вставить("Комментарий", ОбъектМетаданных.Комментарий);
	СвойстваФункциональнойОпции.Описание.Вставить("Пояснение", "");

	СвойстваФункциональнойОпции.Вставить("Синоним", ОбъектМетаданных.Синоним);
	СвойстваФункциональнойОпции.Вставить("Хранение", ОбъектМетаданных.Хранение);
	СвойстваФункциональнойОпции.Вставить("ПривилегированныйРежимПриПолучении", 
		ОбъектМетаданных.ПривилегированныйРежимПриПолучении);

	ПреобразоватьЗначенияСвойствВСтроковоеПредставление(СвойстваФункциональнойОпции);

	Возврат СвойстваФункциональнойОпции;
	
КонецФункции

&НаСервереБезКонтекста
Функция СвойстваПараметраФункциональнойОпции(ИмяПараметраФункциональнойОпции)
	
	СвойстваПараметраФункциональнойОпции = Новый Структура;

	ОбъектМетаданных = Метаданные.ПараметрыФункциональныхОпций[ИмяПараметраФункциональнойОпции];

	СвойстваПараметраФункциональнойОпции.Вставить("Описание", Новый Структура);
	СвойстваПараметраФункциональнойОпции.Описание.Вставить("Комментарий", ОбъектМетаданных.Комментарий);
	СвойстваПараметраФункциональнойОпции.Описание.Вставить("Пояснение", "");

	СвойстваПараметраФункциональнойОпции.Вставить("Синоним", ОбъектМетаданных.Синоним);
	СвойстваПараметраФункциональнойОпции.Вставить("Использование", ОбъектМетаданных.Использование);

	ПреобразоватьЗначенияСвойствВСтроковоеПредставление(СвойстваПараметраФункциональнойОпции);

	Возврат СвойстваПараметраФункциональнойОпции;
	
КонецФункции

&НаСервереБезКонтекста
Функция СвойстваОбщейФормы(ИмяОбщейФормы)

	ОбщаяФорма = Метаданные.ОбщиеФормы[ИмяОбщейФормы];

	СвойстваФормы = Новый Структура;

	СвойстваФормы.Вставить("Описание", Новый Структура);
	СвойстваФормы.Описание.Вставить("Комментарий", ОбщаяФорма.Комментарий);
	СвойстваФормы.Описание.Вставить("Пояснение", ОбщаяФорма.Пояснение);
	
	СвойстваФормы.Вставить("Синоним", ОбщаяФорма.Синоним);
	ТипФормыОбъекта = Новый СписокЗначений;
	ТипФормыОбъекта.Добавить(Строка(ОбщаяФорма.ТипФормы));
	СвойстваФормы.Вставить("Тип", ТипФормыОбъекта);
	СвойстваФормы.Вставить("НазначенияИспользования", ОбщаяФорма.НазначенияИспользования);

	СвойстваФормы.Вставить("Модули", Новый СписокЗначений);
	СвойстваФормы.Модули.Добавить(
		"ОбщаяФорма." + ИмяОбщейФормы + ".Форма.Модуль.txt",
		"Модуль формы: " + ИмяОбщейФормы);
...
Функция СвойстваКонстанты(ИмяКонстанты)
	...
	СвойстваКонстанты.Вставить("Синоним", ОбъектМетаданных.Синоним);
...
Функция СвойстваСправочника(ИмяСправочника)
	...
	СвойстваСправочника.Вставить("Синоним", ОбъектМетаданных.Синоним);
...
Функция СвойстваДокумента(ИмяДокумента)
	...
	СвойстваДокумента.Вставить("Синоним", ОбъектМетаданных.Синоним);
...
Функция СвойстваЖурналаДокумента(ИмяЖурналаДокумента)
	...
	СвойстваЖурналаДокумента.Вставить("Синоним", ОбъектМетаданных.Синоним);
...
Функция СвойстваПеречисления(ИмяПеречисления)
	...
	СвойстваПеречисления.Вставить("Синоним", ОбъектМетаданных.Синоним);
...
Функция СвойстваОтчета(ИмяОтчета)
	...
	СвойстваОтчета.Вставить("Синоним", ОбъектМетаданных.Синоним);
...
Функция СвойстваОбработки(ИмяОбработки)
	...
	СвойстваОбработки.Вставить("Синоним", ОбъектМетаданных.Синоним);
...
Функция СвойстваПланаВидовХарактеристик(ИмяПланаВидовХарактеристик)
	...
	СвойстваПланаВидовХарактеристик.Вставить("Синоним", ОбъектМетаданных.Синоним);
...
Функция СвойстваПланаСчетов(ИмяПланаСчетов)
	...
	СвойстваПланаСчетов.Вставить("Синоним", ОбъектМетаданных.Синоним);
...
Функция СвойстваРегистраСведений(ИмяРегистраСведений)
	...
	СвойстваРегистраСведений.Вставить("Синоним", ОбъектМетаданных.Синоним);
...
Функция СвойстваРегистраНакопления(ИмяРегистраНакопления)
	...
	СвойстваРегистраНакопления.Вставить("Синоним", ОбъектМетаданных.Синоним);
...
Функция СвойстваРегистраБухгалтерии(ИмяРегистраБухгалтерии)
	...
	СвойстваРегистраБухгалтерии.Вставить("Синоним", ОбъектМетаданных.Синоним);
...
Функция СвойстваБизнесПроцесса(ИмяБизнесПроцесса)
	...
	СвойстваБизнесПроцесса.Вставить("Синоним", ОбъектМетаданных.Синоним);
...
Функция СвойстваЗадачи(ИмяЗадачи)
	...
	СвойстваЗадачи.Вставить("Синоним", ОбъектМетаданных.Синоним);
...
Функция СвойстваВнешнийИсточникДанных(ИмяВнешнегоИсточникаДанных)
	...
	СвойстваВнешнегоИсточника.Вставить("Синоним", ОбъектМетаданных.Синоним);
	...
	Иначе
		...
		Иначе
			...
			Если Не ОбъектМетаданных = Неопределено Тогда
				ПредставлениеОбъекта = ПредставлениеОбъектаМетаданных(ОбъектМетаданных);
				ПредставлениеТипа = ПредставлениеОбъекта + "Объект." + ОбъектМетаданных.Имя;
			КонецЕсли;
