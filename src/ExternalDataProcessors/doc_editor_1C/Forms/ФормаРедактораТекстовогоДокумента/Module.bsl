...
Процедура НастроитьКнопкуДобавитьДиаграммуОбъектов(ОбъектМетаданныхСтруктуры)
	...
	Если Не ОбъектМетаданныхСтруктуры.Класс = "Справочник" Тогда
...
Функция ЭтоОбъектСсылочногоТипа(ОбъектМетаданных)
	...
	Возврат ОбъектМетаданных.Класс = "ПланОбмена" Или ОбъектМетаданных.Класс = "Справочник" 
		Или ОбъектМетаданных.Класс = "Документ" Или ОбъектМетаданных.Класс = "Перечисление" 
		Или ОбъектМетаданных.Класс = "ПланВидовХарактеристик" Или ОбъектМетаданных.Класс = "ПланСчетов" 
		Или ОбъектМетаданных.Класс = "ПланВидовРасчета"	Или ОбъектМетаданных.Класс = "БизнесПроцесс" 
...
Функция СвойстваОбъектаМетаданных(ИмяОбъекта, КлассОбъекта)
	...
	ИначеЕсли КлассОбъекта = "МодульОбычногоПриложения" Или КлассОбъекта = "МодульУправляемогоПриложения"
		Или КлассОбъекта = "МодульВнешнегоСоединения" Или КлассОбъекта = "МодульСеанса" Тогда
...
Процедура ДобавитьОпианиеОбъектаВДиаграмму(ОбъектМетаданныхСтруктуры)
	...
	Если Не ЭтоПрикладнойОбъект(ОбъектМетаданныхСтруктуры) Тогда
		Возврат;	 
	КонецЕсли; 
	
	СтрокаМетаданные = СформироватьСтрокуМетаданных("_" + ОбъектМетаданныхСтруктуры.Класс, ОбъектМетаданныхСтруктуры);
	ДокументДобавитьНачалоПараграфа();
	ДокументДобавитьСодержание(СтрокаМетаданные);
...
&НаКлиенте
Функция СформироватьСтрокуМетаданных(ИмяМакросаОбъекта, ОбъектМетаданныхСтруктуры)
...
Процедура ДобавитьСвойстваОбъектаВДиаграмму(ОбъектМетаданныхСтруктуры)
	...
	Для Каждого ПодчиненныйЭлемент Из ОбъектМетаданныхСтруктуры.ПолучитьЭлементы() Цикл
		...
		Если ЭтоПодчиненныеЭлементыОбъекта(ПодчиненныйЭлемент, Истина) Тогда
			...
			ДобавитьОписаниеДанныхОбъектаДиаграммы(ПодчиненныйЭлемент, ОписаниеСвойствОбъекта);
		...
		ИначеЕсли (ПодчиненныйЭлемент.Класс = "ТабличнаяЧасть" И ПодчиненныйЭлемент.МенеджерПодчиненныхЭлементов) Тогда
			...
			Для Каждого ТабличнаяЧастьОбъекта Из ПодчиненныйЭлемент.ПолучитьЭлементы() Цикл
				...
				ОписаниеСвойствОбъекта = ОписаниеСвойствОбъекта 
					+ "--" + ТабличнаяЧастьОбъекта.Имя + "--";
				
				ДобавитьОписаниеДанныхОбъектаДиаграммы(ТабличнаяЧастьОбъекта, ОписаниеСвойствОбъекта);
...
&НаКлиенте
Функция ДобавитьОписаниеДанныхОбъектаДиаграммы(ОбъектМетаданныхСтруктуры, ОписаниеСвойствОбъекта)
	...
	Для Каждого ЭлементДанные Из ОбъектМетаданныхСтруктуры.ПолучитьЭлементы() Цикл
		...
		ОписаниеСвойствОбъекта = ОписаниеСвойствОбъекта + Символы.ПС;
		ОписаниеСвойствОбъекта = ОписаниеСвойствОбъекта + " + " + ЭлементДанные.Имя + ": " + ЭлементДанные.Свойства.Тип;
...
&НаСервереБезКонтекста
Функция СвязанныеОбъекты(КлассОбъекта, ИмяОбъекта)
	
	ОбъектМетаданных = Метаданные.Справочники.тест_Справочник;
	
	Для Каждого Реквизит Из ОбъектМетаданных.Реквизиты Цикл
		
		Если Реквизит.Имя = "Ссылка" Или Реквизит.Имя = "Родитель" Тогда
			Продолжить;	
		КонецЕсли;
		
		ТипыРеквизита = Реквизит.Тип.Типы();
		Для Каждого ТипДанных Из ТипыРеквизита Цикл
			Если ЭтоСсылка(ТипДанных)Тогда
				МетаданныеСвязи = Метаданные.НайтиПоТипу(ТипДанных);
				Сообщить("Справочник связан с " + МетаданныеСвязи.ПолноеИмя());
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	
//	ПланОбмена = Метаданные.ПланыОбмена.ПланОбмена;
//	
//		Для Каждого Реквизит Из ПланОбмена.Реквизиты Цикл
//		
//		Если Реквизит.Имя = "Ссылка" Или Реквизит.Имя = "Родитель" Тогда
//			Продолжить;	
//		КонецЕсли;
//		
//			Если Реквизит.Тип.СодержитТип(Тип("СправочникСсылка.тест_Справочник")) Тогда
//				Сообщить("Справочник связан с " + ПланОбмена.ПолноеИмя());
//			КонецЕсли;
//		
//		КонецЦикла;
...
&НаСервереБезКонтекста
Функция ПодчиненныеСправочники(Парам)
	
	ИмяСправочника = "тест_Справочник";
	
	Для Каждого ПрикладнойОбъектМетаданых Из ПрикладныеОбъектыМетаданных() Цикл
		
		ДанныеОбъекта = ДанныеИспользуемыеОбъектомМетаданных(ПрикладнойОбъектМетаданых);
		Для Каждого ОбъектСправочник Из Метаданные[ПрикладнойОбъектМетаданых] Цикл
			
			Если ДанныеОбъекта.Реквизиты Тогда
			
			Для Каждого Реквизит Из ОбъектСправочник.Реквизиты Цикл
		
				Если Реквизит.Имя = "Ссылка" Или Реквизит.Имя = "Родитель" Тогда
					Продолжить;	
				КонецЕсли;
		
				Если Реквизит.Тип.СодержитТип(Тип("СправочникСсылка." + ИмяСправочника)) Тогда
					Сообщить("Справочник связан с " + ОбъектСправочник.ПолноеИмя());
				КонецЕсли;
