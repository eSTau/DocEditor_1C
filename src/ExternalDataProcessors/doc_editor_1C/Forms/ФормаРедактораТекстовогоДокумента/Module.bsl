...
Процедура ПриОткрытии(Отказ)
	...
	мКаталогФайловИсходников = "C:\Edt_project\1C_editor\ConfigDump\";
...
Функция ОбъектИмеетПрограммныйМодульОбъекта(ОбъектМетаданных)
	...
	Возврат ОбъектМетаданных.Класс = "ОбщийМодуль"
		Или ОбъектМетаданных.Класс = "КритерийОтбора"
		Или ОбъектМетаданных.Класс = "Справочник"
		Или ОбъектМетаданных.Класс = "Константа"
		Или ОбъектМетаданных.Класс = "Документ"
...
&НаКлиенте
Процедура ДобавитьОписаниеПрограммныхМодулей(ОбъектМетаданныхСтруктуры)
	...
	Если СвойстваОбъекта.Свойство("Модули") Тогда
		Для Каждого МодуляОбъекта Из СвойстваОбъекта.Модули Цикл
			КонтекстМодуля = СтруктураКонтекстМодуляОбъекта(МодуляОбъекта.Значение);
			Если Не КонтекстМодуля = Неопределено Тогда
				ДобавитьОписаниеКонтекстаМодуля(КонтекстМодуля, МодуляОбъекта.Представление);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
&НаКлиенте
Процедура ДобавитьОписаниеКонтекстаМодуля(КонтекстМодуля, ПредставлениеМодуля)
	...
	Для Каждого БлокМодуля Из КонтекстМодуля Цикл
		...
		ДокументДобавитьСодержание(
			БлокМодуля.ОписаниеБлока.ТипМетода + " " + БлокМодуля.ОписаниеБлока.ИмяМетода 
			+ ?(БлокМодуля.ОписаниеБлока.Экспортный, " - экспортный метод", ""));
		...
		Если Не БлокМодуля.ОписаниеБлока.ВозвращаемыеЗначения = Неопределено Или Не ПустаяСтрока(
				БлокМодуля.ОписаниеБлока.ПроизвольноеОписаниеВозвращаемыеЗначения) Тогда

			БлокОписанияВозвращаемогоЗначения = БлокОписанияВозвращаемогоЗначения + Символы.ПС + Символы.ПС
				+ "Возвращаемое значение:";
			ДанныеВозвращаемогоЗначения = БлокМодуля.ОписаниеБлока.ВозвращаемыеЗначения;
			Для Каждого ТипВозвращаемогоЗначения Из ДанныеВозвращаемогоЗначения Цикл
				БлокОписанияВозвращаемогоЗначения = БлокОписанияВозвращаемогоЗначения + Символы.ПС + "Тип: "
					+ ТипВозвращаемогоЗначения.Тип;
				БлокОписанияВозвращаемогоЗначения = БлокОписанияВозвращаемогоЗначения + Символы.ПС
					+ ТипВозвращаемогоЗначения.Описание;
			КонецЦикла;

			Если Не ПустаяСтрока(БлокМодуля.ОписаниеБлока.ПроизвольноеОписаниеВозвращаемыеЗначения) Тогда
				БлокОписанияВозвращаемогоЗначения = БлокОписанияВозвращаемогоЗначения + Символы.ПС
					+ БлокМодуля.ОписаниеБлока.ПроизвольноеОписаниеВозвращаемыеЗначенияТогда;
			КонецЕсли;

		КонецЕсли;

		БлокОписанияПримера = "";
		Если Не ПустаяСтрока(БлокМодуля.ОписаниеБлока.Примеры) Тогда

			БлокОписанияПримера = БлокОписанияПримера + Символы.ПС + Символы.ПС + "Примеры:";
			БлокОписанияПримера = БлокОписанияПримера + Символы.ПС + БлокМодуля.ОписаниеБлока.Примеры;

		КонецЕсли;

		БлокОписанияМетода = "";
		Если Не ПустаяСтрока(БлокМодуля.ОписаниеБлока.Назначение) Тогда

			БлокОписанияМетода = БлокОписанияМетода + Символы.ПС + Символы.ПС + "Описание:";
			БлокОписанияМетода = БлокОписанияМетода + Символы.ПС + БлокМодуля.ОписаниеБлока.Назначение;

		КонецЕсли;

//		ОписаниеСиктасиса = ОписаниеСиктасиса + ОписаниеСоставаПараметров + ")" + БлокОписанияПараметров
//			+ БлокОписанияВозвращаемогоЗначения + БлокОписанияПримера + БлокОписанияМетода;

//		МодульОбъекта.ДобавитьСтроку(ОписаниеСиктасиса);
...
Процедура ДобавитьОписаниеСинтексисаМетода(ОписаниеМетода)
	...
	Если Не ОписаниеМетода.ПараметрыМетода = Неопределено И ОписаниеМетода.ПараметрыМетода.Количество() Тогда
		...
		БлокОписанияПараметров = БлокОписанияПараметров 
			+ ДокументБлокНачалоПараграфа() + "**Параметры:**";
		...
		Для Каждого Параметр Из ОписаниеМетода.ПараметрыМетода Цикл
			...
			ОписаниеСоставаПараметров = ОписаниеСоставаПараметров 
				+ ?(ПустаяСтрока(ОписаниеСоставаПараметров), "", ", ");
			...
			БлокОписанияПараметров = БлокОписанияПараметров 
				+ ?(Не ЗначениеЗаполнено(ДанныеПараметра.ЗначениеПоУмолчанию), "(обязательный)", "");
			БлокОписанияПараметров = БлокОписанияПараметров 
				+ ?(ДанныеПараметра.ЗНАЧ, ", передается по значению", "");

			БлокОписанияПараметров = БлокОписанияПараметров + ДокументБлокНачалоПараграфа() + "Тип: ";
			Если Не ДанныеПараметра.ТипыПараметра = Неопределено Тогда
				Для Каждого ТипПараметра Из ДанныеПараметра.ТипыПараметра Цикл
					...
					БлокОписанияПараметров = БлокОписанияПараметров 
						+ ДокументБлокПереносСтроки() + ТипПараметра.Описание;
...
&НаКлиенте
Процедура ОткрытьМодульОбъекта(ИмяМодуля)
	
	НайденныеФайлы = НайтиФайлы(мКаталогФайловИсходников + ИмяМодуля);
	Если НайденныеФайлы.Количество() > 0 Тогда
				
		МодульОбъекта = Новый ТекстовыйДокумент();
		МодульОбъекта.Прочитать(НайденныеФайлы[0].ПолноеИмя);
		МодульОбъекта.Показать();
				
		БлокиМодуля = СтруктураКонтекстМодуляОбъекта("Имя");
		МодульОбъекта.Очистить();
		
		Для Каждого БлокМетода Из БлокиМодуля Цикл
		
			Если Не БлокМетода.ТипБлока = "ЗаголовокМетода" Тогда
				Продолжить;
			КонецЕсли;				
		
			МодульОбъекта.ДобавитьСтроку(БлокМетода.ОписаниеБлока.ТипМетода + " " + БлокМетода.ОписаниеБлока.ИмяМетода 
				+ ?(БлокМетода.ОписаниеБлока.Экспортный, " - экспортный метод", ""));
			
			МодульОбъекта.ДобавитьСтроку("");
			МодульОбъекта.ДобавитьСтроку("Синтаксис:");
			ОписаниеСиктасиса = БлокМетода.ОписаниеБлока.ИмяМетода + "(";

			ОписаниеСоставаПараметров = "";
			БлокОписанияПараметров = "";

			Если Не БлокМетода.ОписаниеБлока.ПараметрыМетода = Неопределено Тогда
				
				БлокОписанияПараметров = БлокОписанияПараметров + Символы.ПС + Символы.ПС + "Параметры:";
				НомерМараметра = 0;
				Для Каждого Параметр Из БлокМетода.ОписаниеБлока.ПараметрыМетода Цикл

					НомерМараметра = НомерМараметра + 1;
					ДанныеПараметра = Параметр.Значение;
					ОписаниеСоставаПараметров = ОписаниеСоставаПараметров + ?(ПустаяСтрока(ОписаниеСоставаПараметров),
						"", ", ");
		//ОписаниеСоставаПараметров = ОписаниеСоставаПараметров + ?(ДанныеПараметра.ЗНАЧ, "Знач ", "");
					ОписаниеСоставаПараметров = ОписаниеСоставаПараметров + "<" + Параметр.Ключ + ">";
		//ОписаниеСоставаПараметров = ОписаниеСоставаПараметров + ?(ЗначениеЗаполнено(ДанныеПараметра.ЗначениеПоУмолчанию), " = " + ДанныеПараметра.ЗначениеПоУмолчанию, "");

					Если НомерМараметра > 1 Тогда
						БлокОписанияПараметров = БлокОписанияПараметров + Символы.ПС;
					КонецЕсли;
					БлокОписанияПараметров = БлокОписанияПараметров + Символы.ПС;
					БлокОписанияПараметров = БлокОписанияПараметров + "<" + Параметр.Ключ + ">";
					БлокОписанияПараметров = БлокОписанияПараметров + ?(Не ЗначениеЗаполнено(
						ДанныеПараметра.ЗначениеПоУмолчанию), "(обязательный)", "");
					БлокОписанияПараметров = БлокОписанияПараметров + ?(ДанныеПараметра.ЗНАЧ,
						", передается по значению", "");

					БлокОписанияПараметров = БлокОписанияПараметров + Символы.ПС + "Тип: ";
					Если Не ДанныеПараметра.ТипыПараметра = Неопределено Тогда
						Для Каждого ТипПараметра Из ДанныеПараметра.ТипыПараметра Цикл
							БлокОписанияПараметров = БлокОписанияПараметров + ТипПараметра.Тип;
							БлокОписанияПараметров = БлокОписанияПараметров + Символы.ПС + ТипПараметра.Описание;
							Если ЗначениеЗаполнено(ДанныеПараметра.ПроизвольноеОписание) Тогда
								БлокОписанияПараметров = БлокОписанияПараметров + Символы.ПС + ТипПараметра.Описание;
							КонецЕсли;
						КонецЦикла;
			...
			БлокОписанияВозвращаемогоЗначения = "";
			Если Не БлокМетода.ОписаниеБлока.ВозвращаемыеЗначения = Неопределено Или Не ПустаяСтрока(
				БлокМетода.ОписаниеБлока.ПроизвольноеОписаниеВозвращаемыеЗначения) Тогда

				БлокОписанияВозвращаемогоЗначения = БлокОписанияВозвращаемогоЗначения + Символы.ПС + Символы.ПС
					+ "Возвращаемое значение:";
				ДанныеВозвращаемогоЗначения = БлокМетода.ОписаниеБлока.ВозвращаемыеЗначения;
				Для Каждого ТипВозвращаемогоЗначения Из ДанныеВозвращаемогоЗначения Цикл
					БлокОписанияВозвращаемогоЗначения = БлокОписанияВозвращаемогоЗначения + Символы.ПС + "Тип: "
						+ ТипВозвращаемогоЗначения.Тип;
					БлокОписанияВозвращаемогоЗначения = БлокОписанияВозвращаемогоЗначения + Символы.ПС
				...
				Если Не ПустаяСтрока(БлокМетода.ОписаниеБлока.ПроизвольноеОписаниеВозвращаемыеЗначения) Тогда
					БлокОписанияВозвращаемогоЗначения = БлокОписанияВозвращаемогоЗначения + Символы.ПС
						+ БлокМетода.ОписаниеБлока.ПроизвольноеОписаниеВозвращаемыеЗначенияТогда;
				КонецЕсли;

			КонецЕсли;

			БлокОписанияПримера = "";
			Если Не ПустаяСтрока(БлокМетода.ОписаниеБлока.Примеры) Тогда

				БлокОписанияПримера = БлокОписанияПримера + Символы.ПС + Символы.ПС + "Примеры:";
				БлокОписанияПримера = БлокОписанияПримера + Символы.ПС + БлокМетода.ОписаниеБлока.Примеры;

			КонецЕсли;

			БлокОписанияМетода = "";
			Если Не ПустаяСтрока(БлокМетода.ОписаниеБлока.Назначение) Тогда

				БлокОписанияМетода = БлокОписанияМетода + Символы.ПС + Символы.ПС + "Описание:";
				БлокОписанияМетода = БлокОписанияМетода + Символы.ПС + БлокМетода.ОписаниеБлока.Назначение;
...
Процедура ПоказатьМодульОбъекта(Команда)
	...
	ОткрытьМодульОбъекта(ИмяФайлаМодуляОбъекта(ТекущийОбъектМетаданных()));
...
&НаКлиенте
Функция СтруктураКонтекстМодуляОбъекта(ИмяМодульОбъекта)
	
	МодульОбъекта = ПрограммныйМодульОбъекта(ИмяМодульОбъекта);
	...
	БлокиМодуля = БлокиПрограммногоМодуля(ТипыБлоковМодуля, МодульОбъекта);
...
//				Назначение = ПолучитьНазначениеБлока(МодульОбъекта, Блок.НачальнаяСтрока, Блок.КонечнаяСтрока);
//
...
//			КонецЕсли; 
		...
		ИначеЕсли Блок.ТипБлока = ТипыБлоковМодуля.НачалоОбласти Тогда
			...
			СтрокаМодуля = СокрЛП(МодульОбъекта.ПолучитьСтроку(Блок.НачальнаяСтрока));
...
// TODO: Если комментарий идущий перед первым разделом считаем назначением модуля.
			...
			Если ЭтоРаздел Тогда
				...
				Если ПоследнийБлокКомментария <> Неопределено Тогда
					...
					Назначение = ПолучитьНазначениеБлока(
						МодульОбъекта, ПоследнийБлокКомментария.НачальнаяСтрока, 
		...
		ИначеЕсли Блок.ТипБлока = ТипыБлоковМодуля.ЗаголовокМетода Тогда
			...
			Если ПоследнийБлокКомментария <> Неопределено Тогда
				ЗаполнитьОписаниеПараметровМетода(МодульОбъекта, ПоследнийБлокКомментария, Блок);
				ПоследнийБлокКомментария.ТипБлока = ТипыБлоковМодуля.Описание;
			КонецЕсли;
		...
		ИначеЕсли Блок.ТипБлока = ТипыБлоковМодуля.ОкончаниеМетода Тогда
			...
			ПоследнийБлокМетода.ОписаниеБлока.Тело = 
				ПолучитьСодержимоеБлока(МодульОбъекта, ПоследнийБлокМетода.КонечнаяСтрока + 1, Блок.НачальнаяСтрока - 1);
			ПоследнийБлокМетода = Неопределено;
...
Функция ПрограммныйМодульОбъекта(ИмяПрограммногоМодуля)
	...
	НайденныеФайлы = НайтиФайлы(мКаталогФайловИсходников + ИмяПрограммногоМодуля);
	Если НайденныеФайлы.Количество() > 0 Тогда
		МодульОбъекта = Новый ТекстовыйДокумент();
		МодульОбъекта.Прочитать(НайденныеФайлы[0].ПолноеИмя);
...
// 
// Параметры:
//  МодульОбъекта - ТекстовыйДокумент - Содержание модуля объекта
// 
// Возвращаемое значение:
//  
&НаКлиенте
Функция БлокиПрограммногоМодуля(ТипыБлоковМодуля, МодульОбъекта)
	
	БлокиМодуля = Новый Массив();

	КоличествоСтрокМодуля = МодульОбъекта.КоличествоСтрок();
	...
	Для НомерСтроки = 1 По КоличествоСтрокМодуля Цикл
		...
		СтрокаМодуля = ВРег(СокрЛП(МодульОбъекта.ПолучитьСтроку(НомерСтроки)));
		...
		Если Не ТекущийБлок = ПредыдущийБлок Тогда
			...
			ЗаполнитьСодержимоеБлокаМодуля(МодульОбъекта, ДанныеБлока, ТипыБлоковМодуля);
...
&НаКлиенте
Процедура ЗаполнитьСодержимоеБлокаМодуля(МодульОбъекта, ДанныеБлока, ТипыБлоковМодуля)
	
	УдалятьКомментарии = ДанныеБлока.ТипБлока = ТипыБлоковМодуля.ЗаголовокМетода;
	ДанныеБлока.Содержимое = ПолучитьСодержимоеБлока(
		МодульОбъекта, ДанныеБлока.НачальнаяСтрока, ДанныеБлока.КонечнаяСтрока, УдалятьКомментарии);
			
КонецПроцедуры

&НаКлиенте
Функция ПолучитьСодержимоеБлока(МодульОбъекта, НачальнаяСтрока, КонечнаяСтрока, УдалятьКомментарии = Ложь)
	
	Строки = Новый Массив();

	Для Ит = НачальнаяСтрока По КонечнаяСтрока Цикл
		
		СтрокаМодуля = МодульОбъекта.ПолучитьСтроку(Ит);
...
&НаКлиенте
Функция ПолучитьНазначениеБлока(МодульОбъекта, Знач НачальнаяСтрока, Знач КонечнаяСтрока, ИмяБлока = "")

	Назначение = "";
	Если НачальнаяСтрока + 1 < КонечнаяСтрока Тогда

		СтрокаМодуляНач = СокрЛП(МодульОбъекта.ПолучитьСтроку(НачальнаяСтрока));
		СтрокаМодуляКон = СокрЛП(МодульОбъекта.ПолучитьСтроку(КонечнаяСтрока));
		Если СтрНачинаетсяС(СтрокаМодуляНач, "////") 
			...
			Если Не ПустаяСтрока(ИмяБлока) Тогда
				...
				СтрокаМодуля2 = СокрЛП(МодульОбъекта.ПолучитьСтроку(НачальнаяСтрока + 1));
			...
			Для Ит = НачальнаяСтрока + 1 По КонечнаяСтрока - 1 Цикл
				...
				СтрокаМодуля = СокрЛП(Сред(МодульОбъекта.ПолучитьСтроку(Ит), 3));
						...
						Иначе
							...
							Если ПозицияКавычки = 0 Или ПозицияКавычки > ПозицияЗапятая Или Не КавычкаОткрыта Тогда
								...
								ЗначениеПараметра = СокрЛП(ЗначениеПараметра 
									+ Лев(СтрокаПараметров, ПозицияЗапятая - 1));
								СтрокаПараметров = СокрЛП(Сред(СтрокаПараметров, ПозицияЗапятая + 1));
								Прервать;
...
&НаКлиенте
Процедура ЗаполнитьОписаниеПараметровМетода(МодульОбъекта, БлокКомментарияМетода, БлокМетода)
	
	ПараметрыМетода = БлокМетода.ПараметрыМетода;
	...
	Для НомерСтроки = БлокКомментарияМетода.НачальнаяСтрока По БлокКомментарияМетода.КонечнаяСтрока Цикл
		...
		СтрокаКомментария = МодульОбъекта.ПолучитьСтроку(НомерСтроки);
		...
		Если ИмяРазделаОписания = "Назначение" Тогда
			...
			ОписаниеБлока.Назначение = ОписаниеБлока.Назначение 
				+ ?(ПустаяСтрока(ОписаниеБлока.Назначение), "", Символы.ПС) + СтрокаКомментария;
					...
					Иначе
						...
						Если ТекущийПараметр.ТипыПараметра.Количество() Тогда 
							ОписаниеТекущегоПараметра = 
								ТекущийПараметр.ТипыПараметра[ТекущийПараметр.ТипыПараметра.ВГраница()];
						КонецЕсли;
					КонецЕсли;
			...
			Иначе
				Если ОписаниеБлока.ВозвращаемыеЗначения.Количество() Тогда
					ВозвращаемоеЗначение = 
						ОписаниеБлока.ВозвращаемыеЗначения[ОписаниеБлока.ВозвращаемыеЗначения.ВГраница()];
				КонецЕсли;
			КонецЕсли;
		...
		ИначеЕсли ИмяРазделаОписания = "Примеры" Тогда
			...
			ОписаниеБлока.Примеры = ОписаниеБлока.Примеры 
				+ ?(ПустаяСтрока(ОписаниеБлока.Примеры), "", Символы.ПС) + СтрокаКомментария;
				...
				Если ПустаяСтрока(ДанныеТипаЗначения.Описание) Тогда
					...
					Для НомерСимвола = 1 По ДлинаСтроки Цикл
						Если Сред(СоставСтрокиОписания[Итератор], НомерСимвола, 1) <> " " Тогда
							СоставСтрокиОписания[Итератор] = 
								СокрЛ(Лев(СоставСтрокиОписания[Итератор], НомерСимвола - 1)) 
								+ ВРег(Сред(СоставСтрокиОписания[Итератор], НомерСимвола, 1)) 
								+ Сред(СоставСтрокиОписания[Итератор], НомерСимвола + 1);
							Прервать;
						КонецЕсли;
					КонецЦикла;
