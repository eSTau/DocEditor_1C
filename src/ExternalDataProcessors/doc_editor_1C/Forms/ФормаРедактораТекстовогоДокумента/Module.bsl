...
#Область ОбработчикиСобытийЭлементовТаблицыФормы_СтруктураМетаданных
...
Процедура ЗаполнитьТипыОбъектаМетаданных(ЭлементОбъектаМетаданных, ОбъектМетаданных)
	...
	ЭлементОбъектаМетаданных.Тип = ТипыОбъектов(ОписаниеТипов.Типы());
...
&НаСервере
Процедура ДобавитьОписаниеФормы(ОбъектМетаданных, КлассаМенеджераОбъекта, ОбъектМетаданныхФорма, ЭлементФорма)
	...
	Для Каждого ВидПрезназначенияФормы Из ВидыПредназначенияФорм Цикл
		Если ОбъектМетаданныхФорма = ОбъектМетаданных[ВидПрезназначенияФормы.Значение] Тогда
			ЭлементФорма.Описание = ?(ПустаяСтрока(ЭлементФорма.Описание), "", ", ")
				+ ВидПрезназначенияФормы.Представление + ".";
		КонецЕсли;
	КонецЦикла;

	ЭлементФорма.Описание = ЭлементФорма.Описание
		+ ?(ПустаяСтрока(ЭлементФорма.Описание), "", " ") + ОбъектМетаданныхФорма.Комментарий;
	
КонецПроцедуры

// TODO: Возможно нужно объединить эту функцию с функцией получения свойств формы
&НаСервере
Функция ВидыПредназначенияФормОбъекта(КлассаМенеджераОбъекта)
	
	ВидыПредназначенияФорм = Новый СписокЗначений();
	
	Если КлассаМенеджераОбъекта = "Справочники" Тогда
	...
	ИначеЕсли КлассаМенеджераОбъекта = "Документы" Тогда
...
Процедура ДобавитьОписаниеОбъектаВДокумент()
	...
	ПредставлениеОбъекта = МаркированноеПредставлениеОбъектаМетаданных(ОбъектМетаданныхСтруктуры, Ложь);
...
//	КонецЕсли;
	...
	МаркерФорматирования = Новый Структура("Маркер, Представление, Объект", 
		ВидРазработки + ПредставлениеОбъекта.Текст, ПредставлениеОбъекта.ВыделяемыйФрагмент, ОбъектМетаданныхСтруктуры);
...
Процедура ДобавитьСвойстваОбъектаВОписание(ОбъектМетаданныхСтруктуры)
	...
	ВывестиДанныеВФорматировануюТаблицу(СвойстваОбъекта);
	
//	ДокументДобавитьЗаголовокТаблицыСвойств();
//	Для Каждого СвойствоОбъекта Из СвойстваОбъекта Цикл
//		Если СвойствоОбъекта.Ключ = "Описание" Тогда
//			Продолжить;
//		КонецЕсли;
//		
//		ДокументДобавитьНовуюСтрокуТаблицы();
//		ДокументДобавитьВТаблицуСтрокуОписанияСвойства(СвойствоОбъекта.Ключ, СвойствоОбъекта.Значение);
//			
//	КонецЦикла;
...
&НаКлиенте
Процедура ВывестиДанныеВФорматировануюТаблицу(СвойстваОбъекта)

	ФорматированнаяТаблица = Новый Массив();
	
	ЗначенияКолонок = СтрРазделить("Свойство|Значение свойства", "|");
	КоличествоКолонок = ЗначенияКолонок.Количество();
	
	ПараметрыКолонокТаблицы = Новый Массив(КоличествоКолонок);
	СтрокаРазделитьельЗаголовка = Новый Массив(КоличествоКолонок);
	ФорматированнаяТаблица.Добавить(ПараметрыКолонокТаблицы);
	ФорматированнаяТаблица.Добавить(ЗначенияКолонок);
	ФорматированнаяТаблица.Добавить(СтрокаРазделитьельЗаголовка);
		
	Для ИндексКолонки = 0 По КоличествоКолонок - 1 Цикл
		ПараметрыКолонокТаблицы[ИндексКолонки] = 0;
	КонецЦикла;
	...
	Для Каждого Элемент Из СвойстваОбъекта Цикл
		...
		Если СтрНайти(Элемент.Ключ, "Группа") Тогда
			СтрокаТаблицы = СтрокаТаблицы + "***" + Элемент.Значение + "***|"; 
		Иначе
			СтрокаТаблицы = СтрокаТаблицы 
				+ "" + Элемент.Ключ + "|" + Элемент.Значение;
		КонецЕсли;
		
		ЗначенияКолонок = СтрРазделить(СтрокаТаблицы, "|", Истина);
		Для ИндексКолонки = 0 По КоличествоКолонок - 1 Цикл
			ПараметрыКолонокТаблицы[ИндексКолонки] = 
				Макс(ПараметрыКолонокТаблицы[ИндексКолонки], 
					СтрДлина(" " + СокрЛП(ЗначенияКолонок[ИндексКолонки] + " ")));
		КонецЦикла;
	...
	Для ИндексСтроки = 1 По ФорматированнаяТаблица.Количество() - 1 Цикл
		Для индексКолонки = 0 По КоличествоКолонок - 1 Цикл
			
			ЗначениеКолонки = ФорматированнаяТаблица[ИндексСтроки][ИндексКолонки];
			ЗначениеКолонки = " " + СокрЛП(ЗначениеКолонки);
			
			ДлинаЗначения = СтрДлина(ЗначениеКолонки);
			Если ДлинаЗначения < ПараметрыКолонокТаблицы[ИндексКолонки] Тогда
				...
				Для КоличествоСимволов = 1 По ПараметрыКолонокТаблицы[ИндексКолонки] - ДлинаЗначения Цикл
			...
			ФорматированнаяТаблица[ИндексСтроки][ИндексКолонки] = ЗначениеКолонки;
	...
	Для ИндексСтроки = 1 По ФорматированнаяТаблица.Количество() - 1 Цикл
		...
		Для индексКолонки = 0 По КоличествоКолонок - 1 Цикл
			РедактируемыйДокумент = РедактируемыйДокумент 
				+ ФорматированнаяТаблица[ИндексСтроки][ИндексКолонки] + "|"; 
		КонецЦикла;
...
Функция СвойстваСправочника(ИмяСправочника)
	...
	СвойстваСправочника.Описание.Вставить("Комментарий", МетаданныеСправочника.Комментарий);
	СвойстваСправочника.Описание.Вставить("Пояснение", МетаданныеСправочника.Пояснение);
	
	СвойстваСправочника.Вставить("ГруппаОсновные", "Основные");
	СвойстваСправочника.Вставить("ДлинаКода", МетаданныеСправочника.ДлинаКода);
	СвойстваСправочника.Вставить("ДлинаНаименования", МетаданныеСправочника.ДлинаНаименования);
	СвойстваСправочника.Вставить("ТипКода", МетаданныеСправочника.ТипКода);
	СвойстваСправочника.Вставить("ОсновноеПредставление", МетаданныеСправочника.ОсновноеПредставление);
	
	СвойстваСправочника.Вставить("ГруппаНумерация", "Нумерация");
	СвойстваСправочника.Вставить("Автонумерация", МетаданныеСправочника.Автонумерация);
	СвойстваСправочника.Вставить("КонтрольУникальности", МетаданныеСправочника.КонтрольУникальности);
	СвойстваСправочника.Вставить("СерииКодов", МетаданныеСправочника.СерииКодов);
	
	СвойстваСправочника.Вставить("ГруппаИерархия", "Иерархия");
	СвойстваСправочника.Вставить("Иерархический", МетаданныеСправочника.Иерархический);
	Если СвойстваСправочника.Иерархический Тогда
		СвойстваСправочника.Вставить("ВидИерархии", МетаданныеСправочника.ВидИерархии);
		СвойстваСправочника.Вставить("КоличествоУровней", МетаданныеСправочника.КоличествоУровней);
	КонецЕсли;
	
	СвойстваСправочника.Вставить("ГруппаПолеВвода", "Поле ввода");
	СвойстваСправочника.Вставить("БыстрыйВыбор", МетаданныеСправочника.БыстрыйВыбор);
	СвойстваСправочника.Вставить("СпособВыбора", МетаданныеСправочника.СпособВыбора);
	СвойстваСправочника.Вставить("ВводПоСтроке", МетаданныеСправочника.ВводПоСтроке);
	
	СвойстваСправочника.Вставить("ГруппаВладельцы", "Владельцы");
	СвойстваСправочника.Вставить("Владельцы", МетаданныеСправочника.Владельцы);
	
	СвойстваСправочника.Вставить("ГруппаПредставление", "Представление");
	СвойстваСправочника.Вставить("ПредставлениеОбъекта", МетаданныеСправочника.ПредставлениеОбъекта);
	СвойстваСправочника.Вставить("РасширенноеПредставлениеОбъекта", МетаданныеСправочника.РасширенноеПредставлениеОбъекта);
	СвойстваСправочника.Вставить("ПредставлениеСписка", МетаданныеСправочника.ПредставлениеСписка);
	СвойстваСправочника.Вставить("РасширенноеПредставлениеСписка", МетаданныеСправочника.РасширенноеПредставлениеСписка);
	
	СвойстваСправочника.Вставить("ГруппаПрочее", "Прочее");
	СвойстваСправочника.Вставить("ПоляБлокировкиДанных", МетаданныеСправочника.ПоляБлокировкиДанных);
	СвойстваСправочника.Вставить("ПолнотекстовыйПоиск", МетаданныеСправочника.ПолнотекстовыйПоиск);
	СвойстваСправочника.Вставить("ИсторияДанных", МетаданныеСправочника.ИсторияДанных);
	...
	ИначеЕсли ТипЗнч(ЗначениеСвойства) = Тип("КоллекцияЗначенийСвойстваОбъектаМетаданных") Тогда
		Для Каждого ОбъектМетаданных Из ЗначениеСвойства Цикл
			Представление = Представление + ?(ПустаяСтрока(Представление), "", ", ") 
				+ ОбъектМетаданных.Имя + "{" + ТипОбъектаМетаданных(ОбъектМетаданных) + "}";
		КонецЦикла;
	Иначе
...
Процедура ДобавитьОписаниеПодчиненныхОбъектов(ОбъектМетаданныхСтруктуры)
	...
	Для Каждого ПодчиненныйЭлемент Из ОбъектМетаданныхСтруктуры.ПолучитьЭлементы() Цикл
		...
		Если ЭтоПодчиненныеЭлементыОбъекта(ПодчиненныйЭлемент) Тогда
			...
			ДокументДобавитьНачалоПараграфа();
			РедактируемыйДокумент = РедактируемыйДокумент  + "**" + ПодчиненныйЭлемент.Синоним + ":**";
			
			ДокументДобавитьЗаголовокТаблицы();
		...
		ИначеЕсли (ПодчиненныйЭлемент.Класс = "ТабличнаяЧасть" 
			...
			Для Каждого ТабличнаяЧастьОбъекта Из ПодчиненныйЭлемент.ПолучитьЭлементы() Цикл
				...
				ДокументДобавитьНачалоПараграфа();
				РедактируемыйДокумент = РедактируемыйДокумент + "**Табличная часть:** " + ТабличнаяЧастьОбъекта.Имя;
...
Процедура ДобавитьОписаниеПодчиненныхОбъектовВДокумент(ОбъектМетаданныхСтруктуры, МенеджерЭлементов)
	...
	Для Каждого Элемент Из МенеджерЭлементов.ПолучитьЭлементы() Цикл
		...
		Если Не ЗначениеЗаполнено(Элемент.Свойства) Тогда
			ЗаполнитьСвойстваПодчиненногоЭлемента(ОбъектМетаданныхСтруктуры, Элемент);
		КонецЕсли;
		...
		ИначеЕсли ТипЗнч(Элемент.Тип) = Тип("СписокЗначений") Тогда
			...
			КоличествоТипов = Элемент.Тип.Количество();
			ИндексТипа = 1;

			ПредставлениеТипа = "";
			Для Каждого Тип Из Элемент.Тип Цикл
				ПредставлениеТипа = ПредставлениеТипа + Тип.Значение + ?(КоличествоТипов = ИндексТипа, "", ";<br> ");
				ИндексТипа = ИндексТипа + 1;
			КонецЦикла;
		...
		ДокументДобавитьНовуюСтрокуТаблицы();
		ДобавитьВТаблицуСтрокуОписанияДанных(Элемент.Имя, ПредставлениеТипа, Элемент.Описание, Элемент);
...
&НаКлиенте
Процедура ЗаполнитьСвойстваПодчиненногоЭлемента(ОбъектМетаданныхСтруктуры, ЭлементОбъекта)
...
&НаСервереБезКонтекста
Функция СвойстваРеквизита(ИмяМенеджераОбъекта, ИмяОбъектаРодителя, ИмяРеквизита)
	...
	СвойстваРеквизита.Описание.Вставить("Подсказка", РеквизитОбъекта.Подсказка);
...
&НаКлиенте
Процедура ОчиститьФрагментТекста()
