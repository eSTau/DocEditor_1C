...
Процедура ДобавитьОписаниеФормы(ОбъектМетаданных, КлассаМенеджераОбъекта, ОбъектМетаданныхФорма, ЭлементФорма)
	...
	ВидыПредназначенияФорм = ВидыПредназначенияФормОбъетка(КлассаМенеджераОбъекта);
...
&НаСервере
Функция ВидыПредназначенияФормОбъетка(КлассаМенеджераОбъекта)
...
Процедура ДобавитьСвойстваОбъектаВОписание(ОбъектМетаданных)
	...
	СвойстваОбъекта = СвойстаОбъектаМетаданных(ОбъектМетаданных.Имя, ОбъектМетаданных.Класс);
	
	ДобавитьНачалаПараграфа();
...
Функция СвойстваСправочника(ИмяСправочника)
	...
	ПреобразоватьЗначениеСвойствВСтроковоеПредставление(СвойстваСправочника);
...
&НаСервереБезКонтекста
Процедура ПреобразоватьЗначениеСвойствВСтроковоеПредставление(СвойстваОбъекта)
	
	Для Каждого СвойствоОбъекта Из СвойстваОбъекта Цикл
		СвойстваОбъекта[СвойствоОбъекта.Ключ] = ПредставлениеСвойствОбъекта(СвойствоОбъекта.Значение);
	КонецЦикла;
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ПредставлениеСвойствОбъекта(ЗначениеСвойства)
...
Процедура ДобавитьОписаниеПодчиненныхОбъектов(ОбъектМетаданных)
	...
	Для Каждого ПодчиненныйЭлемент Из ОбъектМетаданных.ПолучитьЭлементы() Цикл
		...
		Если ЭтоПодчиненныеЭлементыОбъекта(ПодчиненныйЭлемент) Тогда
			...
			ДобавитьОписаниеПолейОбъекта(ПодчиненныйЭлемент);
		...
		ИначеЕсли (ПодчиненныйЭлемент.Класс = "ТабличнаяЧасть" 
			...
			Для Каждого ТабличнаяЧастьОбъекта Из ПодчиненныйЭлемент.ПолучитьЭлементы() Цикл
				...
				ДобавитьЗаголовокТаблицы();
				ДобавитьОписаниеПолейОбъекта(ТабличнаяЧастьОбъекта);
...
&НаКлиенте
Процедура ДобавитьНачалаПараграфа()
...
Процедура ДобавитьЗаголовокТаблицы()
	...
	ДобавитьНачалаПараграфа();
	РедактируемыйДокумент = РедактируемыйДокумент + "Имя | Тип | Описание"
		+ Символ(10) + "---------|----------|---------";
	
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьОписаниеПолейОбъекта(МенеджерПолей)
	...
	Для Каждого Поле Из МенеджерПолей.ПолучитьЭлементы() Цикл
		...
		ДобавитьНовуюСтрукТаблицы();
		ДобавитьВТаблицуСтрокуОписанияДанных(Поле.Имя, ПредставлениеТипа, Поле.Описание);
...
&НаКлиенте
Процедура ДобавитьНовуюСтрукТаблицы()
...
&НаКлиенте
Процедура ДобавитьВТаблицуСтрокуОписанияДанных(Имя, Тип, Описание)
...
Процедура ПодготовитьКонвертерМаркдаун()
	...
	Конвертер.Вставить("ShowDown", 
		ПоместитьВоВременноеХранилище(ФайлShowdown, ЭтаФорма.УникальныйИдентификатор));
	
	ФайлShowdon_map = ОбработкаОбъект.ПолучитьМакет("ShowDownMap");
	Конвертер.Вставить("ShowDownMap", 
		ПоместитьВоВременноеХранилище(ФайлShowdon_map, ЭтаФорма.УникальныйИдентификатор));
...
Процедура РаспаковатьКонвертер()
	...
	ПодключитьКонвертерКОболочкеHtml(ПолноеИмяФайлаJS, КонтекстОболочкиHtml);
	ДокументРезультат = КонтекстОболочкиHtml;				
				
КонецПроцедуры

&НаКлиенте
Процедура ПодключитьКонвертерКОболочкеHtml(ИмяФайлаКонвертера, КонтекстОболочкиHtml)
	
	АдресПодключенияКонвертера = ПолноеИмяФайлаВВидеURL(ИмяФайлаКонвертера);
	КонтекстОболочкиHtml = СтрЗаменить(КонтекстОболочкиHtml, "%%Showdown%%", АдресПодключенияКонвертера);
