...
//	по шапке таблицы определяем сколько колонок будет выводиться.
	ФрагментДокумент = "";
	ФрагментДокумент = ФрагментДокумент 
	+ "|Свойство | Значение свойства|"+ Символ(10) 
	+ "|---|---|" + Символ(10);
	...
	Для Каждого Элемент Из СвойстваОбъекта Цикл
		...
		Иначе
			ФрагментДокумент = ФрагментДокумент 
				+ "|" + Элемент.Ключ + " | " + Элемент.Значение +"|" + Символ(10);
		КонецЕсли;

	КонецЦикла;

	РедактируемыйДокумент = ФрагментДокумент;
