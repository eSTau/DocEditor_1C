...
Процедура ОткрытьМодульОбъекта(ИмяМодуля)
	...
	Если НайденныеФайлы.Количество() > 0 Тогда
		...
		СтруктураМодуля = ПрочитатьМодуль(мКаталогФайловИсходников + ИмяМодуля, Неопределено);
		БлокиМодуля = БлокиПрограммногоМодуля(МодульОбъекта);
...
&НаКлиенте
Функция КонтекстМодуляОбъекта(МодульОбъекта)
...
//			КонецЕсли; 
		...
		ИначеЕсли Блок.ТипБлока = ТипыБлоковМодуля.НачалоОбласти Тогда
			...
			Если ЭтоРаздел И (ОбластиМодуля.Количество() ИЛИ МетодОткрыт) Тогда
		...
		ИначеЕсли Блок.ТипБлока = ТипыБлоковМодуля.ЗаголовокМетода Тогда
			...
			Заголовок = СокрЛП(СтрЗаменить(Заголовок, "PROCEDURE", ""));
			...
			Блок.ОписаниеБлока.ИмяМетода = ИмяМетода;
			Блок.ОписаниеБлока.ПараметрыМетода = ПолучитьПараметрыМетода(СтрокаПараметров);
			Блок.ОписаниеБлока.Экспортный = СтрЗаканчиваетсяНа(ВРег(Заголовок), "ЭКСПОРТ");

			// получим описание метода
			Если ПоследнийБлокКомментария <> Неопределено Тогда

				СтрокаКомментария = МодульОбъекта.ПолучитьСтроку(ПоследнийБлокКомментария.НачальнаяСтрока);
				СтрокаКомментария = СокрЛП(Сред(СтрокаКомментария, 3));
				Если СтрНайти(СтрокаКомментария, ИмяМетода) Тогда // Почему мы проверяем наличие в комментарии имя метода?
					ПоследнийБлокКомментария.ТипБлока = ТипыБлоковМодуля.Описание;
...
&НаКлиенте
Процедура ЗаполнитьОписаниеПараметровМетода(МодульОбъекта, ПоследнийБлокКомментария, БлокМетода)
	
	ДанныеОписанияМетода = ДанныеОписанияМетода(МодульОбъекта, ПоследнийБлокКомментария);

	Если ДанныеОписанияМетода.НомерСтрокиПараметры <> Неопределено Тогда
		...
		Для НомерСтроки = ДанныеОписанияМетода.НомерСтрокиПараметры + 1 По ПоследнийБлокКомментария.КонечнаяСтрока Цикл

			СтрокаКомментария = МодульОбъекта.ПолучитьСтроку(НомерСтроки);
			СтрокаКомментария = СокрЛП(Сред(СтрокаКомментария, 3));
			Если СтрНачинаетсяС(СтрокаКомментария, "Возвращаемое значение:") 
				Или СтрНачинаетсяС(СтрокаКомментария, "Примеры:") Или СтрНачинаетсяС(СтрокаКомментария, "Пример:") Тогда
			...
			Иначе
				...
				Если СоставСтрокиКомментария.Количество() >= 3 Тогда 
					...
					Если ИмяПараметра <> Неопределено Тогда
						...
						СтрокаПараметраМетода = БлокМетода.ОписаниеБлока.ПараметрыМетода.Найти(ИмяПараметра, "Имя");
				...
				ИначеЕсли СоставСтрокиКомментария.Количество() = 2 
					И СтрЗаканчиваетсяНа(СокрЛП(СоставСтрокиКомментария[1]), ",") Тогда
	...
	Если НомерСтрокиВозвращаемоеЗначение <> Неопределено Тогда
		...
		Если ТипПараметра <> "" Тогда
			...
			БлокМетода.ОписаниеБлока.Вставить("ТипВозвращаемогоЗначения", ТипПараметра);
			БлокМетода.ОписаниеБлока.Вставить("ОписаниеВозвращаемогоЗначения", ОписаниеПараметра);						
	...
	Если НомерСтрокиПример <> Неопределено Тогда
		...
		Если Примеры.Количество() Тогда
			...
			БлокМетода.ОписаниеБлока.Вставить("Примеры", Примеры);												

		КонецЕсли;

	КонецЕсли;

	БлокМетода.ОписаниеБлока.Вставить("Назначение", Назначение);
	
КонецПроцедуры

&НаКлиенте
Функция ДанныеОписанияМетода(МодульОбъекта, ПоследнийБлокКомментария)
	
	ДанныеОписанияМетода = Новый Структура(
		"Назначение, НомерСтрокиПараметры, НомерСтрокиВозвращаемоеЗначение, НомерСтрокиПример"
		"", Неопределено, Неопределено, Неопределено);
	
	Для НомерСтроки = ПоследнийБлокКомментария.НачальнаяСтрока + 1 По ПоследнийБлокКомментария.КонечнаяСтрока Цикл

		СтрокаКомментария = МодульОбъекта.ПолучитьСтроку(НомерСтроки);
		СтрокаКомментария = СокрЛП(Сред(СтрокаКомментария, 3));
		Если СтрНачинаетсяС(СтрокаКомментария, "Параметры:") Тогда

			ДанныеОписанияМетода.НомерСтрокиПараметры = НомерСтроки;
			Прервать;

		ИначеЕсли СтрНачинаетсяС(СтрокаКомментария, "Возвращаемое значение:") Тогда

			ДанныеОписанияМетода.НомерСтрокиВозвращаемоеЗначение = НомерСтроки;
			Прервать;

		ИначеЕсли СтрНачинаетсяС(СтрокаКомментария, "Примеры:") 
			ИЛИ СтрНачинаетсяС(СтрокаКомментария, "Пример:") Тогда

			ДанныеОписанияМетода.НомерСтрокиПример = НомерСтроки;
			Прервать;

		Иначе

			ДанныеОписанияМетода.Назначение = ДанныеОписанияМетода.Назначение 
				+ ?(ПустаяСтрока(ДанныеОписанияМетода.Назначение), "", Символы.ПС) + СтрокаКомментария;

		КонецЕсли;

	КонецЦикла;
	
	Возврат ДанныеОписанияМетода;
...
Функция ДанныеБлокаМодуля()
	...
	ДанныеБлока = Новый Структура("ТипБлока, НачальнаяСтрока, КонечнаяСтрока, ОписаниеБлока, Содержимое");

	ДанныеБлока.ОписаниеБлока   = Новый Структура;
	ДанныеБлока.ОписаниеБлока.Вставить("ЭтоРаздел", Ложь);
	ДанныеБлока.ОписаниеБлока.Вставить("ИмяОбластиРодителя", "");
	ДанныеБлока.ОписаниеБлока.Вставить("НазначениеРаздела", "");
	ДанныеБлока.ОписаниеБлока.Вставить("ИмяМетода", "");
	ДанныеБлока.ОписаниеБлока.Вставить("ПараметрыМетода", Неопределено);
	ДанныеБлока.ОписаниеБлока.Вставить("Назначение", "");
	ДанныеБлока.ОписаниеБлока.Вставить("Экспортный", Ложь);
	ДанныеБлока.ОписаниеБлока.Вставить("ТипВозвращаемогоЗначения", "");
	ДанныеБлока.ОписаниеБлока.Вставить("ОписаниеВозвращаемогоЗначения", "");
	ДанныеБлока.ОписаниеБлока.Вставить("Примеры", Новый Массив);
	ДанныеБлока.ОписаниеБлока.Вставить("Тело", "");
...
//
// Параметры:
//   МодульБлока - ТекстовыйДокумент - Код модуля
...
Функция ПрочитатьМодуль(ПутьКФайлу, СтрокаМодуль) Экспорт
	...
	СодержимоеФайла.Прочитать(ПутьКФайлу);	
...
Функция ПолучитьПараметрыМетода(СтрокаПараметров)
	...
	Пока Истина Цикл
		...
		ИмяПараметра   = "";
		ДаныеПараметра = Новый Структура("ЗНАЧ, ЗначениеПоУмолчанию, ТипПараметра, ОписаниеПараметра", 
			Ложь, "", Неопределено, "", "");
...
ПараметрыМетода = Новый Соответствие();
ПараметрыМетода.Вставить("Адреса", Новый Структура("ЗНАЧ, ЗначениеПоУмолчанию, ТипыПараметра, ПроизвольноеОписание")); // ТипПараметра нужно заполнять когда создаются параметры метода
ПараметрыМетода.Вставить("ПараметрыАдминистрированияИБ", Новый Структура("ЗНАЧ, ЗначениеПоУмолчанию, ТипыПараметра, ПроизвольноеОписание"));

МодульОбъекта = Новый ТекстовыйДокумент;
МодульОбъекта.УстановитьТекст(ТекстМодуля);
...
НомерСтрокиПараметры = 6;
НомерСтрокиВозвращаемоеЗначение = 9;
ТекущийПараметр = Неопределено;

Для НомерСтроки = 1 По МодульОбъекта.КоличествоСтрок() Цикл
	...
	Если НомерСтроки >= НомерСтрокиВозвращаемоеЗначение Тогда
		Сообщить("№" + НомерСтроки + " - Строка возврата " + СтрокаКомментария);
		Прервать;
	ИначеЕсли НомерСтроки >= НомерСтрокиПараметры Тогда
		Сообщить("№" + НомерСтроки + " - Строка параметра " + СтрокаКомментария);
		//СоставСтрокиКомментария = СтрРазделить(СтрокаКомментария, "-");
		...
		Для Каждого ПараметрМетода Из ПараметрыМетода Цикл
			Если СтрНачинаетсяС(СтрокаКомментария, ПараметрМетода.Ключ) Тогда
				...
				СтрокаКомментария = СокрЛП(Сред(СтрокаКомментария, ПозицияИмениПараметра + СтрДлина(ПараметрМетода.Ключ) + 1));
				Прервать;
		...
		Если Не ТекущийПараметр = Неопределено Тогда
			...
			Если СоставСтрокиКомментария.Количество() Тогда
				...
				Если СтрНачинаетсяС(СтрокаКомментария, "-") Тогда // Это тип параметра      
					ОписаниеТекущегоПараметра = ТекущийПараметр.ТипыПараметра.Добавить(Новый Структура("Тип, Описание"));
					ОписаниеТекущегоПараметра.ТипПараметра = СоставСтрокиКомментария[0];     
				Иначе 
					Если ТекущийПараметр.ТипыПараметра.Количество() Тогда // Возможно это строка описания ранее найденного типа
				...
				Если Не ОписаниеТекущегоПараметра = Неопределено Тогда
					Для Итератор = 1 По СоставСтрокиКомментария.ВГраница() Цикл
						ОписаниеТекущегоПараметра.Описание = ОписаниеТекущегоПараметра.Описание + "-" + СоставСтрокиКомментария[Итератор];
					КонецЦикла;
				Иначе
...
Процедура ДополнитьБлокиМодуля(БлокиМодуля, Файл, Модуль)
	...
	ОписаниеМодуля = Новый Структура("Глобальный, ЕстьНазначениеМодуля, Назначение,	Разделы",
